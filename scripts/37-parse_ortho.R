...
dat <- fread("/data/users/lgladiseva/annotation_course/scripts/genespace/orthofinder/Results_Nov20/Comparative_Genomics_Statistics/Statistics_PerSpecies.tsv", header = T, fill = TRUE)
...
p <- ggplot(ortho_ratio, aes(x = V1, y = perc, fill = species)) +
    ...
    labs(y = "Count")
ggsave("/data/users/lgladiseva/annotation_course/Plots_orthofinder/orthogroup_plot.pdf")
...
p <- ggplot(ortho_percent, aes(x = V1, y = as.numeric(perc), fill = species)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(y = "Count")
ggsave("/data/users/lgladiseva/annotation_course/Plots_orthofinder/orthogroup_percent_plot.pdf")


library(UpSetR)


orthogroups <- fread("/data/users/lgladiseva/annotation_course/scripts/genespace/orthofinder/Results_Nov20/Orthogroups/Orthogroups.GeneCount.tsv")
...
#pdf("/data/users/dbassi/annotation_course/Plots/one-to-one_orthogroups_plot.complexupset.pdf", height = 5, width = 10, useDingbats = FALSE)
