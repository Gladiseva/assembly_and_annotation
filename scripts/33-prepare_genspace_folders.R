...
# Load the annotation
annotation <- fread("/data/users/lgladiseva/annotation_course/gene_annotation/final/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
...
top20_scaff <- fread("/data/users/lgladiseva/assembly_course/assembly_out/lja/assembly.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/lgladiseva/annotation_course/gene_annotation/final/assembly.all.maker.fasta.all.maker.proteins.fasta.renamed.filtered.longest.fasta"
